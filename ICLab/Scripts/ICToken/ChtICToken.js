				...
				if (ret.func != null) {
					switch(ret.func) {
						...
						case "bulidUnblockCardRequest":
							...
							break;
						case "unblockCardCheckNewPIN": //Virtual
						case "unblockCard":
							Gbjct.RspDt=event.data;
							l_vRetData=UrRstUBlkInfo(GARevFun.UrFUN);						
							break;						
						case "generalUnblockCard":
							Gbjct.RspDt=event.data;
							l_vRetData=UrRstGnrlUBlkInfo(GARevFun.UrFUN);						
							break;
						...
						case "buildOpenCardGetUserPINRequest":
							...
							break;					
						case "openCardCheckNewPIN": //Virtual
						case "openCard":
							Gbjct.RspDt=event.data;
...
function getInfo(){
	...
	PreData["keyseqid"]=InToken.keyseqid;
						...
						{													
							if (InToken.SmrtCrdID[InToken.ActvSltID[0]]==slots[index].token.serialNumber) {
								...
								for(var indexKey in keys){								
									...
									(l_vExp.test(indexKey)==true && GARevFun.Para==99 )
									){
										if (l_CertInfo!=null)
											l_CertInfo=l_CertInfo+',"KeyB64":"' + keys[indexKey].keyb64+'"'+',"KeyIDB64":"' + keys[indexKey].id +'"';
										else
											l_CertInfo='"KeyB64":"' + keys[indexKey].keyb64+'"'+',"KeyIDB64":"' + keys[indexKey].id +'"';
...
function UsrGtRstInfo(CardSystem,UID,UrRevFUN) 
{
	...
	InToken.UID=UID;
	if (CardSystem.indexOf('Virtual') != -1){
		InToken.CrdSys=CardSystem;
	}else{
		InToken.CrdSys=CardSystem.toUpperCase(); //compatibility for the past
	}
...
function UsrRstUBlkInfo(CardSystem,UID,ServerRetData,UrRevFUN)
{
	...
	l_vRMsg="";
	if (CardSystem.indexOf('Virtual') != -1){
		InToken.FUN="unblockCardCheckNewPIN";  //Virtual  
		InToken.NewPIN=NewPIN;
	}else{
		InToken.FUN="unblockCard";  //Original
	}	
	InToken.UID=UID;
	if (CardSystem.indexOf('Virtual') != -1){
		InToken.CrdSys=CardSystem;
...
//ePKI Function
//-------------------------------------------------------------------------------------------------------------------
//110 GeneralUnblockCard
function UsrRstGnrlUBlkInfo(SOPIN,NewPIN,UrRevFUN)
{
	var l_iRetCode=0;
	var l_CertInfo="";
	var l_vExp;
	...
	l_vRMsg="";
	InToken.FUN="generalUnblockCard";
	InToken.SOPIN=SOPIN;
	InToken.NewPIN=NewPIN;
	GARevFun.FUN="UrRstGnrlUBlkInfo";
	GARevFun.Para="";
	//GARevFun.PopTout=900000;
	GARevFun.UrFUN=UrRevFUN;	
	...
	if (l_iRetCode==0) {
		SendData("http://localhost:61161/CardManagement/sendCardCommand","http://localhost:61161/CardManagement/CMCPopForm"
				,"IC卡讀取中",Gbjct.PppFrmStyle,UrRstGnrlUBlkInfo,UrRevFUN);
	} else {
		l_vRetData='{"RCode":"' + l_iRetCode +'","RMsg":"'+ l_vRMsg + '"}';
		InToken.RetObj=JSON.parse(l_vRetData);
		if (UrRevFUN && typeof(UrRevFUN) === "function") UrRevFUN();
...
//111
function UrRstGnrlUBlkInfo(UrRevFUN)
{
	var l_vExp;
	var l_iRetCode;
	var l_CertInfo;
...
//91
function UsrPnCrdInfo(CardSystem,UID,ServerRetData,UrRevFUN,Step, NewPIN)   //20211228 add NewPIN: its default is undefined
{
	var l_iRetCode=0;
	var l_CertInfo="";
	var l_vExp;
	...
	if (Step==1) {
		...
		if (CardSystem.indexOf('Virtual') != -1){
			InToken.FUN="openCardCheckNewPIN";  //Virtual  
			InToken.NewPIN=NewPIN;
		}else{
			InToken.FUN="openCard";  //Original
		}
	}//if (Step==1)
...
//object.CertInfo
//object.RMsg
ICToken.prototype.parseCertificate = function (index,RevFun,keyidb64,keyseqid)
{
	if (typeof (keyidb64) === "undefined") keyidb64="__NotExistedTag__";	
	if (typeof (keyseqid) === "undefined") keyseqid="__NotExistedTag__";	
...
//112
ICToken.prototype.resetGnrlUBlkInfo= function (SOPIN,NewPIN,RevFun)
{
	UsrRstGnrlUBlkInfo(encodeURIComponent(SOPIN),encodeURIComponent(NewPIN),RevFun)	
}//ICToken.prototype.resetUBlkInfo = function (index,RevFun)
...
ICToken.prototype.openCrdInfo= function (CardSystem,UID,ServerRetData,RevFun,Step,NewPIN) //20211228 add NewPIN: its default is undefined
{
	//UsrPnCrdInfo(CardSystem,encodeURIComponent(UID),ServerRetData,RevFun,Step);	
	UsrPnCrdInfo(CardSystem,encodeURIComponent(UID),ServerRetData,RevFun,Step, NewPIN);   //20211228 add NewPIN: its default is undefined	 
